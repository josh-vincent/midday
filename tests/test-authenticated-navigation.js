...
async function testAuthenticatedNavigation() {
  ...
  try {
    ...
    if (!emailField || !passwordField) {
      console.log(
        "   ⚠️  Traditional login fields not found - OAuth login only",
      );
      console.log(
        "   📝 The application uses OAuth authentication (Google Sign-In)",
      );
      ...
      console.log(
        "   Run: npm run db:seed or create user via Supabase dashboard",
      );
      ...
      if (submitButton) {
        ...
        if (currentUrl.includes("/login")) {
          ...
          for (const pageInfo of pages) {
            ...
            const screenshotName = pageInfo.name
              .toLowerCase()
              .replace(/ /g, "-");
            await page.screenshot({
            ...
            console.log(
              `   ✓ ${pageInfo.name} page loaded and screenshot saved`,
            );
