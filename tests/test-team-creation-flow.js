...
async function testTeamCreationFlow() {
  ...
  try {
    ...
    try {
      ...
      console.log(
        "⚠️  Login form not found or login failed:",
        loginError.message,
      );
    ...
    try {
      const nameInput = await page
        .locator(
          'input[name="full_name"], input[placeholder*="name" i], input[name="name"]',
        )
        .first();
      if (await nameInput.isVisible({ timeout: 3000 })) {
        ...
        await page.screenshot({
          path: "screenshots/04-after-name-submission.png",
        });
      } else {
    ...
    try {
      ...
      const teamNameInput = await page
        .locator(
          'input[name="team_name"], input[name="name"], input[placeholder*="team" i], input[placeholder*="company" i]',
        )
        .first();
      ...
      if (await teamNameInput.isVisible({ timeout: 5000 })) {
        ...
        const countrySelect = await page
          .locator(
            'select[name="country"], select[name="location"], div:has-text("country") >> select, div:has-text("location") >> select',
          )
          .first();
        ...
        if (await countrySelect.isVisible({ timeout: 3000 })) {
          ...
          const countryDropdown = await page
            .locator(
              '[role="combobox"]:has-text("country"), [role="listbox"]:has-text("country"), button:has-text("country")',
            )
            .first();
    ...
    const errorElements = await page
      .locator('[role="alert"], .error, .alert-error, text=error')
      .all();
    ...
    console.log(
      `✅ Teams page reached: ${finalUrl.includes("/teams") ? "YES" : "NO"}`,
    );
    console.log(`✅ Team creation attempted: ${teamCreated ? "YES" : "NO"}`);
    console.log(
      `✅ Success indicators: ${successFound ? "FOUND" : "NOT FOUND"}`,
    );
    console.log(`📍 Final URL: ${finalUrl}`);
