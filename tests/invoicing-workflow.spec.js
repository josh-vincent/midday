...
async function runCompleteWorkflowTest(page, runNumber) {
  ...
  try {
    ...
    await page.screenshot({
      path: `test-results/login-page-${runNumber.replace(" ", "-").toLowerCase()}.png`,
    });
    ...
    if (isOnLoginPage) {
      ...
      const emailInputs = await page
        ...
        .all();
      console.log(
        `🔐 ${runNumber}: Found ${emailInputs.length} email input fields`,
      );
      ...
      const passwordInputs = await page
        ...
        .all();
      console.log(
        `🔐 ${runNumber}: Found ${passwordInputs.length} password input fields`,
      );
      ...
      const loginButtons = await page
        ...
        .all();
      console.log(
        `🔐 ${runNumber}: Found ${loginButtons.length} login buttons`,
      );
      ...
      if (emailInputs.length > 0 && passwordInputs.length > 0) {
        try {
          ...
          if (loginButtons.length > 0) {
            ...
            console.log(
              `🔐 ${runNumber}: Login successful: ${loginSuccessful}`,
            );
            ...
            if (!loginSuccessful) {
              ...
              const errorElements = await page
                .locator(
                  '[class*="error"], [data-testid*="error"], .alert-error, [role="alert"]',
                )
                .all();
              for (const errorEl of errorElements) {
                ...
                if (errorText && errorText.trim()) {
                  console.log(
                    `❌ ${runNumber}: Login error found: ${errorText.trim()}`,
                  );
                }
              }
            }
    ...
    await page.screenshot({
      path: `test-results/after-login-${runNumber.replace(" ", "-").toLowerCase()}.png`,
    });
    ...
    const customerElements = await page
      .locator('h1, h2, h3, [data-testid*="customer"], [class*="customer"]')
      .all();
    console.log(
      `👥 ${runNumber}: Found ${customerElements.length} customer-related elements`,
    );
    ...
    for (let i = 0; i < Math.min(customerElements.length, 5); i++) {
      ...
      if (text && text.trim()) {
        console.log(
          `👥 ${runNumber}: Customer element ${i + 1}: ${text.trim()}`,
        );
      }
    }

    // Look for Add/New customer button
    const addCustomerButtons = await page
      .locator(
      ...
      .all();
    console.log(
      `👥 ${runNumber}: Found ${addCustomerButtons.length} Add/New buttons`,
    );
    ...
    if (addCustomerButtons.length > 0) {
      try {
        ...
        console.log(
          `❌ ${runNumber}: Error clicking Add customer button: ${clickError.message}`,
        );
      }
    } else {
    ...
    await page.screenshot({
      path: `test-results/customers-page-${runNumber.replace(" ", "-").toLowerCase()}.png`,
    });
    ...
    const invoiceElements = await page
      .locator('h1, h2, h3, [data-testid*="invoice"], [class*="invoice"]')
      .all();
    console.log(
      `📄 ${runNumber}: Found ${invoiceElements.length} invoice-related elements`,
    );
    ...
    for (let i = 0; i < Math.min(invoiceElements.length, 5); i++) {
      ...
      if (text && text.trim()) {
        console.log(
          `📄 ${runNumber}: Invoice element ${i + 1}: ${text.trim()}`,
        );
      }
    }

    // Look for Add/New invoice button
    const addInvoiceButtons = await page
      .locator(
        'button:has-text("Add"), button:has-text("New"), button:has-text("Create"), ' +
          'a:has-text("Add"), a:has-text("New"), a:has-text("Create"), ' +
          '[data-testid*="add"], [data-testid*="new"], [data-testid*="create"]',
      ...
      .all();
    console.log(
      `📄 ${runNumber}: Found ${addInvoiceButtons.length} Add/New buttons`,
    );
    ...
    if (addInvoiceButtons.length > 0) {
      try {
        ...
        console.log(
          `❌ ${runNumber}: Error clicking Add invoice button: ${clickError.message}`,
        );
      }
    } else {
    ...
    await page.screenshot({
      path: `test-results/invoices-page-${runNumber.replace(" ", "-").toLowerCase()}.png`,
    });
    ...
    await page.screenshot({
      path: `test-results/error-${runNumber.replace(" ", "-").toLowerCase()}.png`,
    });
  }
}
