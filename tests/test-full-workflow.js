  ...
  try {
    ...
    if (emailInput && passwordInput) {
      ...
      if (submitButton) {
        ...
        if (isDisabled) {
          console.log(
            "   ⚠️  Submit button is disabled, trying to enable it...",
          );
        ...
        await Promise.race([
          page
            ...
            .catch(() => {}),
          page
            .waitForSelector(".text-red-500", { timeout: 5000 })
            .catch(() => {}),
          delay(8000),
        ]);
        ...
        if (!currentUrl.includes("/login")) {
          ...
          if (onCustomersPage) {
            ...
            const addCustomerButton =
              (await page.$(
                'div.hidden.sm\\:block button[variant="outline"]',
              )) ||
            ...
            if (
              addCustomerButton !== -1 &&
              typeof addCustomerButton === "number"
            ) {
              ...
              if (allButtons.length > 0) {
                ...
                for (
                  ...
                  i >= Math.max(0, allButtons.length - 3);
                  i--
                ) {
                  const btnText = await allButtons[i].evaluate(
                    (el) => el.textContent,
                  );
                  console.log(`   Checking button ${i}: "${btnText}"`);

                  // Check if button has an SVG child (icon button)
                  const hasIcon = await allButtons[i].evaluate(
                  ...
                  if (hasIcon) {
                    ...
                    if (modalOpened) {
                      ...
                      const dialogButtons = await page.$$(
                        '[role="dialog"] button, .sheet-content button',
                      );
                      let saveButton = null;
                      ...
                      if (saveButton) {
                        ...
                        console.log(
                          "   ⚠️  Could not find Save button in dialog",
                        );
                        results.failed.push(
                          "Create customer - save button not found",
                        );
                      }
                      break;
                    }
            ...
            if (onInvoicesPage) {
              ...
              if (allInvoiceButtons.length > 0) {
                for (
                  let i = allInvoiceButtons.length - 1;
                  i >= Math.max(0, allInvoiceButtons.length - 3);
                  i--
                ) {
                  const hasIcon = await allInvoiceButtons[i].evaluate(
                    (el) => !!el.querySelector("svg"),
                  );
                  if (hasIcon) {
                    ...
                    if (invoiceModalOpened) {
                      ...
                      // Submit invoice
                      const invoiceDialogButtons = await page.$$(
                        '[role="dialog"] button, .sheet-content button',
                      );
                      ...
                      if (saveInvoiceButton) {
                        ...
                        console.log(
                          "   ⚠️  Could not find Save button in invoice dialog",
                        );
                        results.failed.push(
                          "Create invoice - save button not found",
                        );
                      }
                      break;
                    }
