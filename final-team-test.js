...
async function finalTeamTest() {
  ...
  try {
    ...
    const existingTeams = await page
      .locator('[data-testid*="team"], .team-item, .team-card')
      .count();
    console.log(`Found ${existingTeams} existing team elements`);

    // Look for "Test Company"
    const testCompanyExists = await page
      .locator("text=Test Company")
      .isVisible();
    ...
    if (await createTeamButton.isVisible({ timeout: 5000 })) {
      ...
      await page.screenshot({
        path: "screenshots/final-04-create-team-clicked.png",
      });
      ...
      if (await nameInput.isVisible({ timeout: 3000 })) {
        ...
        await page.waitForTimeout(3000);
        await page.screenshot({
          path: "screenshots/final-06-after-submit.png",
        });
      } else {
    ...
    console.log(
      `✅ Create team button: ${(await createTeamButton.isVisible()) ? "PRESENT" : "NOT FOUND"}`,
    );
    console.log(`📍 Final URL: ${finalUrl}`);
  } catch (error) {
