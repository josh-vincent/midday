...
async function completeFlow() {
  ...
  try {
    ...
    await page.screenshot({ path: path.join(screenshotsDir, 'flow-01-initial.png') });
    ...
    await page.screenshot({ path: path.join(screenshotsDir, 'flow-02-login-filled.png') });
    ...
    await page.screenshot({ path: path.join(screenshotsDir, 'flow-03-after-login.png') });
    ...
    const isTeamSetup = await page.$('input[placeholder*="Acme Marketing" i]') !== null;
    
    if (isTeamSetup) {
      console.log('✅ On team setup page, filling form');
      
      // Clear and fill company name
      const companyInput = await page.$('input[placeholder*="Acme Marketing" i]');
      await companyInput.click({ clickCount: 3 }); // Select all text
      await companyInput.type('Test Company');
      
      await page.screenshot({ path: path.join(screenshotsDir, 'flow-04-team-filled.png') });
      ...
      await page.screenshot({ path: path.join(screenshotsDir, 'flow-05-after-team-creation.png') });
      ...
      const pageContent = await page.evaluate(() => {
        return {
          ...
          buttons: Array.from(document.querySelectorAll('button')).map(b => b.textContent.trim()).filter(t => t.length > 0),
          links: Array.from(document.querySelectorAll('a')).map(a => ({ text: a.textContent.trim(), href: a.href })).filter(l => l.text.length > 0)
        };
      });
    ...
    const navElements = await page.$$eval('nav a, [role="navigation"] a, [data-testid*="nav"] a', 
    ...
    const invoiceNavigation = navElements.find(nav => 
    ...
    if (invoiceNavigation) {
      ...
      await new Promise(resolve => setTimeout(resolve, 2000));
      await page.screenshot({ path: path.join(screenshotsDir, 'flow-06-invoices.png') });
      ...
      if (navElements.length > 0) {
        ...
        await new Promise(resolve => setTimeout(resolve, 2000));
        await page.screenshot({ path: path.join(screenshotsDir, 'flow-06-main-nav.png') });
      }
    }
    
    // Final screenshot
    await page.screenshot({ path: path.join(screenshotsDir, 'flow-07-final.png') });
    ...
    const finalState = await page.evaluate(() => {
      return {
        ...
        actionButtons: Array.from(document.querySelectorAll('button')).map(b => b.textContent.trim()).filter(t => t.length > 0 && t.length < 50)
      };
    });
    ...
    if (browser) {
      ...
      if (page) {
        await page.screenshot({ path: path.join(__dirname, 'screenshots', 'flow-error.png') });
