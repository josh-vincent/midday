...
export class PDFGenerator {
  ...
  private renderReportContent(doc: PDFKit.PDFDocument, content: ReportContent): void {
    switch (content.type) {
      case "text":
  ...
  private renderText(doc: PDFKit.PDFDocument, text: string, style?: TextStyle): void {
    const currentOptions: any = {};
    
    if (style) {
      if (style.fontSize) doc.fontSize(style.fontSize);
      if (style.fontFamily) doc.font(style.fontFamily);
      if (style.color) doc.fillColor(style.color);
      if (style.alignment) currentOptions.align = style.alignment;
      if (style.lineHeight) currentOptions.lineGap = style.lineHeight;
  ...
  private renderImage(doc: PDFKit.PDFDocument, image: ImageData): void {
    const options: any = {};
    
    if (image.width) options.width = image.width;
    if (image.height) options.height = image.height;
  ...
  private renderList(doc: PDFKit.PDFDocument, list: ListData): void {
    const renderItems = (items: typeof list.items, level = 0) => {
  ...
  private addReportHeader(doc: PDFKit.PDFDocument, header: any): void {
    // Implementation for report header
  }

  private addReportFooter(doc: PDFKit.PDFDocument, footer: any): void {
    // Implementation for report footer
  }

  private applyWatermark(doc: PDFKit.PDFDocument): void {
  ...
  async generateQRCode(data: string, options?: Partial<BarcodeOptions>): Promise<Buffer> {
    const qrOptions = {
  ...
  async generateBarcode(data: string, options?: Partial<BarcodeOptions>): Promise<Buffer> {
  ...
  async addPageNumbers(pdf: Buffer): Promise<Buffer> {
    const pdfDoc = await PDFLib.load(pdf);
