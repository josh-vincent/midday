...
async function testApplication() {
  ...
  try {
    ...
    const formElements = await page.evaluate(() => {
      const elements = {
        ...
        loginText: document.body.textContent.toLowerCase().includes('login') || document.body.textContent.toLowerCase().includes('sign in'),
    ...
    for (const route of protectedRoutes) {
      ...
      try {
        ...
        console.log(`Route ${route} - Status: ${response.status()}, Final URL: ${finalUrl}`);
  ...
  testResults.redirects.forEach(redirect => {
    if (redirect.from && redirect.to) {
      ...
      console.log(`    Status: ${redirect.status}, Redirects to login: ${redirect.redirectsToLogin}`);
    }
  });
  ...
  if (testResults.errors.length > 0) {
    ...
    testResults.errors.forEach((error, index) => {
      console.log(`  ${index + 1}. ${error.url || error.route || 'General'}: ${error.status || 'N/A'} - ${error.message || error.statusText || error.error}`);
    });
  }
  ...
  if (testResults.errors.length === 0 && testResults.consoleErrors.length === 0 && testResults.networkErrors.length === 0) {
