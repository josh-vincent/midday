...
import { JobItemSearch } from "./job-item-search";
...
export function LineItems() {
  ...
  const handleBulkJobsSelected = (
    ...
    // Store job IDs if they should be marked as invoiced
    if (options?.markAsInvoiced) {
      const currentJobIds = form.watch("linkedJobIds") || [];
      const newJobIds = jobs.map((j) => j.id);
      form.setValue("linkedJobIds", [...currentJobIds, ...newJobIds]);
    }

    // Store options for later use during invoice creation
    if (options?.updateOnInvoiceCreation) {
      form.setValue("updateJobsOnCreation", true);
    }

    if (options?.descriptionFields) {
      form.setValue("jobDescriptionFields", options.descriptionFields);
    }

    // Add each selected job as a line item
    jobs.forEach((job) => {
      append({
        name: job.name,
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-2">
        ...
        <JobSearchDialog onJobsSelected={handleBulkJobsSelected} />
      </div>
    </div>
  );
}
