...
export function Form() {
  ...
  // Job linking mutation for when jobs are added to invoice
  const linkJobsToInvoiceMutation = useMutation(
    trpc.job.linkToInvoice.mutationOptions({
      onSuccess: (data) => {
        console.log(`Linked ${data.count} jobs to invoice`);
      ...
      onError: (error) => {
        console.error("Failed to link jobs to invoice:", error);
      },
    }),
  );
  ...
  const createInvoiceMutation = useMutation(
    trpc.invoice.create.mutationOptions({
      onSuccess: (data) => {
        ...
        // Link jobs to invoice if linkedJobIds are present in the form
        const linkedJobIds = form.getValues("linkedJobIds");
        if (linkedJobIds && linkedJobIds.length > 0 && data.id) {
          console.log(`Linking ${linkedJobIds.length} jobs to invoice ${data.id}`);

          linkJobsToInvoiceMutation.mutate({
            jobIds: linkedJobIds,
            invoiceId: data.id,
