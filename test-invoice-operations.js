...
async function testInvoiceOperations() {
  console.log('🚀 Testing Invoice Operations (Status Changes, Downloads, etc.)\n');
  ...
  try {
    ...
    const [draftInvoice] = await sql`
      INSERT INTO invoices (
        ...
        ${JSON.stringify([
          { description: 'Dirt Receiving Service', quantity: 100, price: 5000, total: 500000 }
        ])},
        NOW(), NOW()
      )
      RETURNING *
    ...
    console.log(`      Amount: $${(draftInvoice.total_amount / 100).toFixed(2)}`);
    ...
    console.log(`      💵 Paid: $${(partiallyPaidInvoice.paid_amount / 100).toFixed(2)} / $${(partiallyPaidInvoice.total_amount / 100).toFixed(2)}`);
    ...
    const [duplicatedInvoice] = await sql`
      ...
      RETURNING *
    `;
    console.log(`   ✅ Duplicated invoice: ${duplicatedInvoice.invoice_number}`);
    console.log(`      Status: ${duplicatedInvoice.status} (reset to draft)`);
    console.log(`      Same amount: $${(duplicatedInvoice.total_amount / 100).toFixed(2)}`);
    ...
    const [overdueInvoice] = await sql`
      INSERT INTO invoices (
        ...
        100000, 100000, 'USD',
        ${JSON.stringify([
          { description: 'Overdue Service', quantity: 1, price: 100000, total: 100000 }
        ])},
        NOW(), NOW()
      )
      RETURNING *
    ...
    const [markedOverdue] = await sql`
      ...
      RETURNING *
    `;
    console.log(`   ✅ Created overdue invoice: ${markedOverdue.invoice_number}`);
    ...
    statusDistribution.forEach(s => {
      console.log(`      ${s.status}: ${s.count} invoices, $${(s.total_value / 100).toFixed(2)} total`);
    });
    ...
    console.log(`      Partially Paid: ${paymentPerformance.partial_count} invoices`);
    console.log(`      Total Collected: $${(paymentPerformance.total_collected / 100).toFixed(2)}`);
    console.log(`      Total Outstanding: $${(paymentPerformance.total_outstanding / 100).toFixed(2)}`);
    ...
    const [scheduledInvoice] = await sql`
      INSERT INTO invoices (
        ...
        150000, 150000, 'USD',
        ${JSON.stringify([
          { description: 'Future Service', quantity: 1, price: 150000, total: 150000 }
        ])},
        NOW(), NOW()
      )
      RETURNING *
  ...
  console.log('   • GET  /api/download/invoice?id={id}&preview=true - Preview PDF');
