...
async function debugLogin() {
  ...
  try {
    ...
    console.log('2. Cookies before login:', cookiesBefore.filter(c => c.name.includes('auth')).length);
    ...
    const authResponsePromise = page.waitForResponse(
      response => response.url().includes('auth') || response.url().includes('supabase'),
      { timeout: 10000 }
    ).catch(() => null);
    ...
    if (authResponse) {
      console.log(`5. Auth response: ${authResponse.status()} from ${authResponse.url()}`);
      const responseBody = await authResponse.text().catch(() => 'Could not read body');
      if (responseBody.includes('error')) {
        console.log('   Response contains error:', responseBody.substring(0, 200));
      }
    } else {
    ...
    console.log('6. Cookies after login:', cookiesAfter.filter(c => c.name.includes('auth')).length);
    cookiesAfter.filter(c => c.name.includes('auth')).forEach(c => {
