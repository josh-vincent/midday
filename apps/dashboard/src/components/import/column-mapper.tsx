...
const TARGET_FIELDS = [
  // Required fields
  { field: "ticketNumber", label: "Ticket Number", dataType: "string", required: true },
  { field: "truckRego", label: "Truck Registration", dataType: "string", required: true },
  { field: "grossWeight", label: "Gross Weight (tonnes)", dataType: "number", required: true },
  { field: "tareWeight", label: "Tare Weight (tonnes)", dataType: "number", required: true },
  
  // Optional fields
  { field: "weighInTime", label: "Weigh In Time", dataType: "date", required: false },
  { field: "weighOutTime", label: "Weigh Out Time", dataType: "date", required: false },
  { field: "materialType", label: "Material Type", dataType: "string", required: false },
  { field: "siteFrom", label: "Site From", dataType: "string", required: false },
  { field: "siteTo", label: "Site To", dataType: "string", required: false },
  { field: "purchaseOrder", label: "Purchase Order", dataType: "string", required: false },
  { field: "costCenter", label: "Cost Center", dataType: "string", required: false },
  { field: "customerName", label: "Customer Name", dataType: "string", required: false },
  { field: "rate", label: "Rate per Tonne", dataType: "number", required: false },
  { field: "epaLevyRate", label: "EPA Levy Rate", dataType: "number", required: false },
] as const;
...
export function ColumnMapper({
    ...
    mappings.forEach((csvHeader, targetField) => {
      ...
      if (fieldDef) {
        columnMappings.push({
          ...
          dataType: fieldDef.dataType as "string" | "number" | "date" | "boolean",
  ...
  const getSampleValue = (csvHeader: string, index: number = 0): string => {
    ...
    <div className="space-y-4">
      <div className="rounded-lg bg-muted/50 p-4">
        ...
        <p className="text-sm text-muted-foreground">
          Match your CSV columns to the corresponding invoice fields. Required fields are marked with an asterisk.
        </p>
      </div>
              ...
              <div className="grid gap-4 md:grid-cols-3">
                ...
                <div className="space-y-2">
                  <Select
                    ...
                    onValueChange={(value) => handleMappingChange(targetField.field, value)}
                  >
                    <SelectTrigger>
      ...
      <div className="flex items-center justify-between">
        ...
        <Button
          ...
          disabled={mappings.size < TARGET_FIELDS.filter(f => f.required).length}
        >
