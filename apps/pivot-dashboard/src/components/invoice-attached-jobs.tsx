...
export function InvoiceAttachedJobs({ lineItems, currency }: InvoiceAttachedJobsProps) {
  ...
  const jobItems = lineItems.filter((item) => item.jobId);
    ...
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="w-[350px]">
        ...
        {jobItems.map((item, index) => (
          <DropdownMenuItem key={item.jobId || index} className="flex flex-col items-start py-2">
            <div className="flex justify-between w-full">
              <span className="font-medium">{item.name}</span>
              <span className="text-muted-foreground">
                {formatAmount({ currency, amount: item.total || (item.price * item.quantity) })}
              </span>
            </div>
            {item.description && (
        ...
        <div className="px-2 py-2">
          <div className="flex justify-between text-sm font-medium">
            ...
            <span>
              {formatAmount({ 
                currency, 
                amount: jobItems.reduce((sum, item) => sum + (item.total || (item.price * item.quantity)), 0)
              })}
            </span>
          </div>
        </div>
