    ...
    if (!currentUrl.includes('/login')) {
      ...
      if (page.url().includes('/customers')) {
        ...
        for (let i = buttons.length - 1; i >= 0; i--) {
          const hasIcon = await buttons[i].evaluate(el => !!el.querySelector('svg'));
          if (hasIcon) {
      ...
      if (page.url().includes('/invoices')) {
        ...
        for (let i = invoiceButtons.length - 1; i >= 0; i--) {
          const hasIcon = await invoiceButtons[i].evaluate(el => !!el.querySelector('svg'));
          if (hasIcon) {
...
async function runTestLoop() {
  ...
  if (totalFailed === 0) {
    ...
    console.log('The issue appears to be with the client-side form submission or session handling.');
  }
  console.log('='.repeat(60));
}
