...
async function fixUserTeam() {
  ...
  try {
    ...
    if (adminUser.teamId && adminUser.usersOnTeams.length === 0) {
      console.log(
        `User has teamId ${adminUser.teamId} but no team membership. Fixing...`,
      );
      ...
      if (!team) {
        console.log(
          `Team ${adminUser.teamId} doesn't exist. Clearing user's teamId...`,
        );
        // Clear the teamId
        await db
          .update(schema.users)
        ...
        console.log(
          "User's teamId cleared. User should now be redirected to team creation.",
        );
      } else {
        console.log(
          `Team ${adminUser.teamId} exists. Adding user as member...`,
        );
      ...
      console.log(
        "User has no teamId. They will be redirected to team creation.",
      );
    } else {
