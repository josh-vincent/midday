  ...
  try {
    ...
    const [invoice] = await sql`
      INSERT INTO invoices (
        ...
        ${JSON.stringify([
          {
            description: "Dirt Removal Service",
            quantity: 50,
            price: 5000,
            total: 250000,
          },
    ...
    try {
      ...
      if (response.ok) {
        console.log(
          `   ✅ Public invoice page accessible (Status: ${response.status})`,
        );
        ...
        if (viewedInvoice.viewed_at) {
          console.log(
            `   ✅ Invoice view tracked at: ${viewedInvoice.viewed_at}`,
          );
        }
      } else {
        console.log(
          `   ⚠️ Public invoice page returned status: ${response.status}`,
        );
      }
    } catch (error) {
      console.log(
        `   ⚠️ Could not test public URL (server may need to be running)`,
      );
    }
    ...
    `;
    console.log(
      `   ✅ Marked invoice as partially paid: $${(partiallyPaid.paid_amount / 100).toFixed(2)} / $${(partiallyPaid.total_amount / 100).toFixed(2)}`,
    );
    ...
    summaryByStatus.forEach((s) => {
      console.log(
        `      ${s.status}: ${s.count} invoices, Total: $${(s.total / 100).toFixed(2)}, Avg: $${(s.average / 100).toFixed(2)}`,
      );
    });
    ...
    if (paymentTiming.avg_days_to_payment) {
      console.log(
        `   ✅ Average days to payment: ${Math.round(paymentTiming.avg_days_to_payment)} days`,
      );
    }
    ...
    topCustomers.forEach((c, idx) => {
      console.log(
        `      ${idx + 1}. ${c.name}: ${c.invoice_count} invoices, $${(c.total_revenue / 100).toFixed(2)}`,
      );
    });
