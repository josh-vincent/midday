...
async function testCreateTeamButton() {
    ...
    try {
        ...
        const createTeamButton = await page.locator('button:has-text("Create team"), a:has-text("Create team")').first();
        ...
        const teamNameInput = await page.locator('input[name="name"], input[placeholder*="team"], input[placeholder*="company"]').first();
        ...
        if (await teamNameInput.isVisible({ timeout: 5000 })) {
            ...
            try {
                const countrySelect = await page.locator('select[name="country"], [role="combobox"]').first();
