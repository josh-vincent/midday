...
async function finalNavigationTest() {
  ...
  try {
    ...
    const hasGoogleButton = await page
      .$eval("button", (el) => el.textContent.includes("Continue with Google"))
      .catch(() => false);
    const hasOtherOptions = await page
      .$eval("button", (el) => el.textContent.includes("Other options"))
      .catch(() => false);
    ...
    for (const route of protectedRoutes) {
      await page.goto(`http://localhost:3333${route.path}`, {
        waitUntil: "networkidle2",
      });
      ...
      if (currentUrl.includes("/login")) {
        results.passed.push(
          `${route.name} redirects to login when unauthenticated`,
        );
        ...
        if (currentUrl.includes("return_to")) {
          console.log(
            `      └─ Preserves return URL: ${currentUrl.split("return_to=")[1]}`,
          );
        }
      } else {
        results.failed.push(`${route.name} accessible without authentication`);
        console.log(
          `   ❌ ${route.name}: NOT SECURED (accessible without login)`,
        );
      }
    }
  ...
  if (results.failed.length === 0) {
    ...
    console.log(
      "✅ The application navigation and security are working correctly.",
    );
  } else {
