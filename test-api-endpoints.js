...
async function testAPIEndpoints() {
  ...
  try {
    ...
    console.log(
      `   ✅ Created invoice: ${newInvoice.invoice_number} (ID: ${invoiceId})`,
    );
    console.log(`      Status: ${newInvoice.status}`);
    console.log(
      `      Total Amount: $${(newInvoice.total_amount / 100).toFixed(2)}`,
    );
    ...
    const testInvoices = [
      {
        invoice_number: `INV-${timestamp}-002`,
        status: "unpaid",
        total: 750000,
      }, // $7500.00
      { invoice_number: `INV-${timestamp}-003`, status: "paid", total: 320000 }, // $3200.00
      {
        invoice_number: `INV-${timestamp}-004`,
        status: "overdue",
        total: 1200000,
      }, // $12000.00
      {
        invoice_number: `INV-${timestamp}-005`,
        status: "draft",
        total: 180000,
      }, // $1800.00
    ];
    ...
    for (const inv of testInvoices) {
      const invoiceData = {
        ...
        paid_date:
          inv.status === "paid" ? new Date().toISOString().split("T")[0] : null,
        currency: "USD",
        line_items: JSON.stringify([
          {
      ...
      console.log(
        `   ✅ Created ${created.invoice_number}: Status=${created.status}, Amount=$${(created.total_amount / 100).toFixed(2)}`,
      );
    }
    ...
    statusCounts.forEach((s) => {
      console.log(
        `      ${s.status.toUpperCase()}: ${s.count} invoices, Total: $${(s.total_amount / 100 || 0).toFixed(2)}`,
      );
    });
    ...
    `;
    console.log(
      `   ✅ Total paid revenue: $${(totalRevenue.total / 100 || 0).toFixed(2)}`,
    );
    ...
    `;
    console.log(
      `   ✅ Total outstanding: $${(outstanding.total / 100 || 0).toFixed(2)}`,
    );
