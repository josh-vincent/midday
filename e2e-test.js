...
async function runE2ETest() {
  ...
  try {
    ...
    console.log(
      `Found ${emailInputs.length} email inputs and ${passwordInputs.length} password inputs`,
    );
    ...
    if (emailInputs.length > 0 && passwordInputs.length > 0) {
      ...
      if (loginButtons.length > 0) {
        ...
        await page.screenshot({
          path: path.join(screenshotsDir, "02-no-login-button.png"),
        });
      }
    } else {
      console.log("⚠️  No login form found on initial load");
      await page.screenshot({
        path: path.join(screenshotsDir, "02-no-login-form.png"),
      });
    }
    ...
    if (teamButton) {
      ...
      await new Promise((resolve) => setTimeout(resolve, 2000));
      await page.screenshot({
        path: path.join(screenshotsDir, "04-team-creation-page.png"),
      });

      // Look for team name input
      const teamNameInputs = await page.$$(
        'input[name*="team"], input[placeholder*="team" i], input[placeholder*="company" i]',
      );
      ...
      if (teamNameInputs.length > 0) {
        ...
        await page.screenshot({
          path: path.join(screenshotsDir, "05-team-name-filled.png"),
        });
        ...
        if (createButtons.length > 0) {
          ...
          await new Promise((resolve) => setTimeout(resolve, 3000));
          await page.screenshot({
            path: path.join(screenshotsDir, "06-after-team-creation.png"),
          });
      ...
      await page.screenshot({
        path: path.join(screenshotsDir, "04-no-team-creation.png"),
      });
    }
    ...
    const navItems = await page.$$eval(
      'nav a, [role="navigation"] a, .navigation a, .menu a',
      (elements) =>
        elements
          .map((el) => ({
            text: el.innerText.trim(),
    ...
    const invoiceLinks = navItems.filter(
      (item) =>
    ...
    if (invoiceLinks.length > 0) {
      ...
      await new Promise((resolve) => setTimeout(resolve, 2000));
      await page.screenshot({
        path: path.join(screenshotsDir, "07-invoices-page.png"),
      });
    } else {
    ...
    // Final screenshot
    await page.screenshot({
      path: path.join(screenshotsDir, "08-final-screenshot.png"),
    });
